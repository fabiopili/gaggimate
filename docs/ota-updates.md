...
- The version must compare strictly greater by semver than what is installed, for the display and the controller independently. Re-publishing the same tag is a no-op; every iteration needs a new tag. Since the 2026-08-19 recovery the machine runs display `v1.8.15` and controller `v1.8.13`, both current upstream content on the fork channel, so the next release must be `v1.8.16` or higher. The numbers differ only because the controller was updated from the published `v1.8.13` while the display was flashed over USB from a tree two doc commits later; the firmware is the same. A `v1.8.14` tag exists locally on the abandoned `bridge-display` branch and must never be pushed: it was a display image built from `v1.8.11` content, numbered above the published `v1.8.13` so it would not update itself mid-repair. Delete it and the branch once the machine has been stable for a while.
