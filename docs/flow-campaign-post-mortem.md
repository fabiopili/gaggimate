...
`v1.8.11` is an annotated tag on upstream master `9e6a69bf` (2026-08-07) plus one CI-only commit (`713ae712`). The pristine tag could not build on a fork: upstream's `upload-firmware` action posts firmware to their private update server using secrets only their repo carries, and with those absent the step fails and kills the build before the GitHub release job runs (the Node 20 deprecation notice in the same log is an unrelated warning). The added commit makes that upload skip cleanly when no server is configured, touching nothing outside `.github`, so the firmware source stays identical to upstream; the guard is itself a reasonable patch to offer upstream, since it is what lets any fork build at all. Pushed to the fork, the fork's Actions build the tag with upstream's own workflow (which triggers on any tag) and publish a release on `fabiopili/gaggimate`; the machine's current firmware points its OTA at the fork and updates only when the remote version is strictly newer, which v1.8.11 over v1.8.10 satisfies. The flashed firmware is pure upstream, so from then on the machine looks at `jniebuhr/gaggimate` releases. One consequence to know: upstream's newest tag is v1.8.1, so the machine will not see an upstream OTA until upstream's numbering passes v1.8.11; a USB flash of any genuine upstream release rejoins the official version line at any time. The `fork-ota` branch, all tags and this history remain on the fork for any future resumption.
